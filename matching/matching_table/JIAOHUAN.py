...
def write_model_table(sheet_name, write_data, path, col_data):
    ...
    write_table(worksheet, col_data[10], 11)
    write_table(worksheet, col_data[11], 12)
...
def matching_jhj():
    file_name = r'D:\githubproject\excel\files\汇总表\交换机.xlsx'  # 模板文件地址
    sheet_name = 'host'
    save_path = r'D:\githubproject\excel\files\匹配表\交换机.xls'  # 最终匹配文件存储地址

    hz_file_name = r'D:\githubproject\excel\files\result\交换机.xls'  # 中间表格地址
    hz_sheet_name = '交换机'  # 中间表格sheet名
