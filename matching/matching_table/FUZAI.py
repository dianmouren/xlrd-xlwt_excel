...
def write_model_table(sheet_name, write_data, path, col_data):
    ...
    write_table(worksheet, col_data[6], 17)
    write_table(worksheet, col_data[9], 8)
    write_table(worksheet, col_data[10], 13)
    write_table(worksheet, col_data[11], 19)
    write_table(worksheet, col_data[12], 18)
    write_table(worksheet, col_data[13], 3)
    write_table(worksheet, col_data[14], 7)
    write_table(worksheet, col_data[15], 15)
...
def matching_fz():
    file_name = r'D:\githubproject\excel\files\汇总表\负载均衡.xlsx'  # 模板文件地址
    sheet_name = 'host'
    save_path = r'D:\githubproject\excel\files\匹配表\负载均衡.xls'  # 最终匹配文件存储地址

    hz_file_name = r'D:\githubproject\excel\files\result\负载均衡.xls'  # 中间表格地址
    hz_sheet_name = '负载均衡'  # 中间表格sheet名
